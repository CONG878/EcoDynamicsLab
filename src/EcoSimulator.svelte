<script>
    ...
    function initializeChart() {
        ...
        ecosystemChart = new Chart(ctx, {
            ...
            data: {
                labels: [],
                datasets: [
                    { label: state.names.prey, data: [], borderColor: "rgba(75, 192, 192, 1)", borderWidth: 2, fill: false, yAxisID: "y" },
                    { label: state.names.LP, data: [], borderColor: "rgba(255, 99, 132, 1)", borderWidth: 2, fill: false, yAxisID: "y1" },
                    { label: state.names.AP, data: [], borderColor: "rgba(54, 162, 235, 1)", borderWidth: 2, fill: false, yAxisID: "y1" },
                ],
            },
            options: {
                responsive: true,
                scales: {
                    x: { type: "linear", title: { display: true, text: "관찰 기간(개월)" }, ticks: { stepSize: 3 } },
                    y: { title: { display: true, text: "피식자 개체수" }, beginAtZero: true, position: "left" },
                    y1: { title: { display: true, text: "포식자 개체수" }, beginAtZero: true, position: "right", grid: { display: false } },
                },
            },
        });
    }
    ...
    function randomize() {
        ...
        state.preyTraits.reproductionRate = logGaussianRandom(state.preyTraits.reproductionRate, Math.log(1.25));
        state.preyTraits.carryingCapacity = logGaussianRandom(state.preyTraits.carryingCapacity, Math.log(1.25));

        state.predatorTraits.reproductionRate = state.predatorTraits.reproductionRate.map(rate =>
            logGaussianRandom(rate, Math.log(1.25))
        );
        state.predatorTraits.predationRate = state.predatorTraits.predationRate.map(rate =>
            logGaussianRandom(rate, Math.log(1.25))
        );
        state.predatorTraits.efficiency = state.predatorTraits.efficiency.map(eff =>
            logGaussianRandom(eff, Math.log(1.25))
        );
        state.predatorTraits.alternativeFood = state.predatorTraits.alternativeFood.map(food =>
            logGaussianRandom(food, Math.log(1.25))
        );

        const keys = Object.keys(state.aggressionRateMatrix); // 모든 행의 키 가져오기
        keys.forEach((key) => {
            state.aggressionRateMatrix[key] = state.aggressionRateMatrix[key].map((rate) =>
                logGaussianRandom(rate, Math.log(1.25))
            );
        });
    }
    ...
    function updateChart(simulationResults) {
        ecosystemChart.data.labels = simulationResults.map((result) => result.time.toFixed(2));
        ecosystemChart.data.datasets[0].data = simulationResults.map((result) => result.populations[0]);
        ecosystemChart.data.datasets[1].data = simulationResults.map((result) => result.populations[1]);
        ecosystemChart.data.datasets[2].data = simulationResults.map((result) => result.populations[2]);
        ecosystemChart.update();
    }
...
<div class="root-container">
    <form on:reset={resetForm} on:submit={handleFormSubmit} novalidate>
        <fieldset>
            ...
            <input id="lower-predator-name" type="text" bind:value="{state.names.LP}">
            <label for="apex-predator-name">상위 포식자</label>
            <input id="apex-predator-name" type="text" bind:value="{state.names.AP}">
        </fieldset>
        ...
        <fieldset>
            ...
            <input id="T" type="number" step="any" bind:value="{state.tRange}">
        </fieldset>

        <fieldset>
            <legend>피식자 특성</legend>
            <label for="r0">번식률</label>
            <input id="r0" bind:value="{state.preyTraits.reproductionRate}" type="number" step="any">
            <label for="K0">포화 개체수</label>
            <input id="K0" bind:value="{state.preyTraits.carryingCapacity}" type="number" step="any">
        </fieldset>
        ...
        <fieldset>
            ...
            <fieldset class="r">
                ...
                <input id="r1" bind:value="{state.predatorTraits.reproductionRate[0]}" type="number" step="any">
                <label for="r2">{state.names.AP}</label>
                <input id="r2" bind:value="{state.predatorTraits.reproductionRate[1]}" type="number" step="any">
            </fieldset>
            <fieldset class="a">
                <legend>포식 강도</legend>
                <label for="a1">{state.names.LP}</label>
                <input id="a1" bind:value="{state.predatorTraits.predationRate[0]}" type="number" step="any">
                <label for="a2">{state.names.AP}</label>
                <input id="a2" bind:value="{state.predatorTraits.predationRate[1]}" type="number" step="any">
            </fieldset>
            <fieldset class="b">
                <legend>포식 효율</legend>
                <label for="b1">{state.names.LP}</label>
                <input id="b1" bind:value="{state.predatorTraits.efficiency[0]}" type="number" step="any">
                <label for="b2">{state.names.AP}</label>
                <input id="b2" bind:value="{state.predatorTraits.efficiency[1]}" type="number" step="any">
            </fieldset>
            <fieldset class="c">
                <legend>대체 식량 자원</legend>
                <label for="c1">{state.names.LP}</label>
                <input id="c1" bind:value="{state.predatorTraits.alternativeFood[0]}" type="number" step="any">
                <label for="c2">{state.names.AP}</label>
                <input id="c2" bind:value="{state.predatorTraits.alternativeFood[1]}" type="number" step="any">
            </fieldset>
            <fieldset class="A">
                <legend>포식자 간 공격성</legend>
                <div>
                    <label for="a11">{state.names.LP} → {state.names.LP}</label>
                    <input id="a11" bind:value="{state.aggressionRateMatrix['0'][0]}" type="number" step="any">
                    <label for="a12">{state.names.LP} → {state.names.AP}</label>
                    <input id="a12" bind:value="{state.aggressionRateMatrix['0'][1]}" type="number" step="any">
                </div>
                <div>
                    <label for="a21">{state.names.AP} → {state.names.LP}</label>
                    <input id="a21" bind:value="{state.aggressionRateMatrix['1'][0]}" type="number" step="any">
                    <label for="a22">{state.names.AP} → {state.names.AP}</label>
                    <input id="a22" bind:value="{state.aggressionRateMatrix['1'][1]}" type="number" step="any">
                </div>
            </fieldset>
        </fieldset>
        ...
        <fieldset>
            ...
            <textarea id="description" bind:value="{experimentMetadata.description}"></textarea>
    ...
    <output>
        ...
        <div class="grid-mode">
            ...
            <input type="checkbox" id="prey-axis" bind:checked="{yGridVisible}" on:change="{toggleGrid}">
            <label for="predator-axis">포식자 눈금</label>
            <input type="checkbox" id="predator-axis" bind:checked="{y1GridVisible}" on:change="{toggleGrid}">
        </div>
    </output>
</div>